...
from lbm.utils.dataclass_utils import load_dataclass
...
class DynamicReplicaDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        traj_2d = torch.from_numpy(traj_2d[:, visible_inds_resampled]) # (T, N, 2)
        ...
        query_points = torch.cat(
            [
                torch.zeros_like(traj_2d[0, :, :1]), # (N, 1)
                traj_2d[0, :, 1:2], # (N, 1)
                traj_2d[0, :, 0:1], # (N, 1)
            ],
            dim=1
        ...
        return video, traj_2d, visibility, query_points