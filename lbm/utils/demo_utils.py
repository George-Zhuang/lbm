...
def load_video_3d(video_path, video_depth_path, max_depth=80, click_query=True):
    # Load original video frames and query points
    frames, video, query_points_tensor = load_video(video_path, click_query)
    
    # Load depth video
    depth_frames = []
    cap_depth = cv2.VideoCapture(video_depth_path)
    if not cap_depth.isOpened():
        raise IOError(f"Unable to open depth video file: {video_depth_path}")
    ...
    while True:
        ...
        # Convert grayscale values (0-255) back to actual depth values (0-max_depth)
        depth_metric = (depth_gray.astype(np.float32) / 255.0) * max_depth
        depth_frames.append(depth_metric)
    cap_depth.release()
    
    if not depth_frames:
        raise IOError(f"Could not read any frames from depth video: {video_depth_path}")
    
    # Convert depth frames to tensor
    videodepth_np = np.stack(depth_frames, axis=0)  # T H W
    videodepth = torch.from_numpy(videodepth_np).float()
    videodepth = videodepth.unsqueeze(1)  # T 1 H W
    videodepth = videodepth.unsqueeze(0)  # 1 T 1 H W
    
    # Check if video and depth video have matching frame counts
    if video.shape[1] != videodepth.shape[1]:
        raise ValueError(f"Video frame count ({video.shape[1]}) does not match depth video frame count ({videodepth.shape[1]})")
    
    return frames, video, videodepth, query_points_tensor
