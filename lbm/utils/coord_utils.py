...
def get_queries_3d(traj, vis, traj_depth):
    # traj: (B, T, N, 2)
    # vis: (B, T, N)
    # depth: (B, T, N) in meters

    # returns: (B, N, 3) where 3 is (frame_ind, x, y)

    B, T, N, D = traj.shape
    device = traj.device
    traj_depth = traj_depth.unsqueeze(-1)
    ...
    gather_d = torch.gather(traj_depth, 1, first_positive_inds[:, :, None, None].repeat(1, 1, N, 1)) # (B, T, N, 1)
    xys_d = torch.diagonal(gather_d, dim1=1, dim2=2).permute(0, 2, 1) # (B, N, 1)
    queries = torch.cat([queries, xys_d], dim=2)    # (B, N, 4)
